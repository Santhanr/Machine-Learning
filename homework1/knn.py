...
def compute_distances(Xtrain, X):
	...
	#####################################################
	#				 YOUR CODE HERE					#
	dists = np.empty((X.shape[0],Xtrain.shape[0]),float)
	for i in range(0,X.shape[0]):
		for j in range(0,Xtrain.shape[0]):
			temp=np.subtract(X[i],Xtrain[j])
			temp=np.square(temp)
...
def predict_labels(k, ytrain, dists):
	...
	#####################################################
	#				 YOUR CODE HERE					#
	ypred = np.empty(dists.shape[0], int)
	for i in range(0,dists.shape[0]):
		d={}
		for j in range(0,dists.shape[1]):
			d[str(j)]=dists[i][j]
		d_sorted = sorted(d.items(), key=lambda kv:(kv[1],kv[0]))
		labels=[]
		counter=0
		for ele in d_sorted:
			labels.append(ytrain[int(ele[0])])
			counter=counter+1
			if(counter==k):
				break
		hash = {}
...
def compute_accuracy(y, ypred):
	...
	for i in range(0,y.shape[0]):
		if (y[i]==ypred[i]):
			hits=hits+1
	acc=float(hits)/float(y.shape[0])
	#####################################################
	return acc
...
def find_best_k(K, ytrain, dists, yval):
	...
	validation_accuracy=[]
	for i in K:
		prediction=predict_labels(i,ytrain,dists)
		temp=compute_accuracy(yval,prediction)
